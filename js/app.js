(() => {
    ...
    let startElement = 0;
    let endElement = 3;
    const blogItemsElement = document.querySelector(".blog__items");
    if (blogItemsElement) loadBlogItems();
    async function loadBlogItems() {
        ...
        if (response.ok) {
            ...
            data = responseResult;
            initBlog(data, startElement, endElement);
        } else console.log("Error response");
    }
    function initBlog(data, start, end) {
        console.log(data);
        let dataPaste = data.items.slice(start, end);
        dataPaste.forEach((element => {
    ...
    function buildBlogItem(item) {
        ...
        if (item.image) blogItemTemplate += `<a href="${item.url}" class="item-blog__image-ibg">\n        <img src="${item.image}" alt="img">\n    </a>`;
        blogItemTemplate += `<div class="item-blog__date">${item.date}</div>\n    `;
        blogItemTemplate += `<h4 class="item-blog__titel">\n    <a href="${item.url}" class="item-blog__link-titel">${item.title}</a>\n    </h4>`;
        blogItemTemplate += `<div class="item-blog__text text">\n    <p>\n        ${item.text.slice(0, 45)}...\n    </p>\n</div>`;
        if (item.tags) {
            blogItemTemplate += `<div class="item-blog__tags">`;
            for (const key in item.tags) blogItemTemplate += `<a href="${item.tags[key]}" class="item-blog__tag">${key}</a>`;
            blogItemTemplate += `</div>`;
        }
        blogItemTemplate += `</article>  `;
        console.log(blogItemTemplate);
        blogItemsElement.insertAdjacentHTML("beforeend", blogItemTemplate);
    }
    document.addEventListener("click", documentAction);
    function viewMore() {
        const dataItemsLength = data.items.length;
        const currentItems = document.querySelectorAll(".item-blog").length;
        const btnViewMore = document.querySelector(".blog__view-more");
        currentItems < dataItemsLength ? btnViewMore.hidden = false : btnViewMore.hidden = true;
    }
    function documentAction(e) {
        const targetElement = e.target;
        if (targetElement.closest(".blog__view-more")) {
            startElement = document.querySelectorAll(".item-blog").length;
            endElement = startElement + 3;
            initBlog(data, startElement, endElement);
            e.preventDefault();
        }
    }
